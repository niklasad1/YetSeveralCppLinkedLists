...
template <typename T> unsigned int LinkedList<T>::size() const noexcept {
...
template <typename T> T const LinkedList<T>::get(const unsigned int &index) {
  unsigned int i = 0;
...
template <typename T> void LinkedList<T>::addFirst(const T &elem) noexcept {
...
template <typename T> void LinkedList<T>::addLast(const T &elem) noexcept {
  if (!head && !tail) {
    head = tail = new Node(elem, nullptr, nullptr);
  } else {
...
template <typename T> auto LinkedList<T>::remove(const T &elem) -> T const {
  try {
...
template <typename T> auto LinkedList<T>::removeFirst() -> Node * {
  if (!head && !tail) {
...
template <typename T> auto LinkedList<T>::removeLast() -> Node * {
  if (!head && !tail) {
...
bool LinkedList<T>::containsHelper(Node *curr, const T &elem) const noexcept {
  if (!curr)
    ...
    return containsHelper(curr->next, elem);
}
...
auto LinkedList<T>::removeHelper(Node *curr, const T &elem) -> Node * {
  if (!curr) {
    ...
    return removeHelper(curr->next, elem);
}

template <typename T> auto LinkedList<T>::find(const T &elem) -> Node * {
  for (auto it = head; it != nullptr; it = it->next) {
    if (it->value == elem)
      return it;
  }
